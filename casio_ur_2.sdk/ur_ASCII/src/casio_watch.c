...
* VERSION HISTORY
********************************************************************************************
*	v1.0 - 16/11/2001	-	Johannes C. Thomsen
*		First version created.
*
*	v1.1 -17/11
*		ASCII art added to the project	-	Johannes C. Thomsen / Bjørn Patterson
*******************************************************************************************/
...
//Start address in array from ascii.h
#define START_ADDRESS_0 0
#define START_ADDRESS_1 7
#define START_ADDRESS_2 14
#define START_ADDRESS_3 21
#define START_ADDRESS_4 28
#define START_ADDRESS_5 35
#define START_ADDRESS_6 42
#define START_ADDRESS_7 49
#define START_ADDRESS_8 56
#define START_ADDRESS_9 63
#define START_ADDRESS_10 70
...
enum states {CLOCK, ALARM_SETTING, SET_ALARM_HOUR, SET_ALARM_MINUTES, ALARM_IS_ACTIVE ,STOPWATCH ,
			LAP ,STOPWATCH_RUNNING , SETTINGS , SET_CLOCK_HOUR , SET_CLOCK_MINUTE , SET_MONTH , SET_DAY, SET_DATE , SET_YEAR};	//Watch states
...
void setYear();
void printAsciiNumber(int hour10, int hour0, int min10, int min0,  int sec10,  int sec0);
int getStartAddress(int number);
...
int main (void)
{
  ...
  XUartPs_SetBaudRate(&uart, baudrate);	//set the baud rate
  while(1){}
  return 0;
}
...
// Control the speed of the watch for debugging purposes with the switch interrupt handler
void SW_Intr_Handler(void *InstancePtr){
	...
	sw_value = XGpio_DiscreteRead(&SWInst, 1);	//read the value of the switch
...
// Button interrupt handler, handles all the scenarios, when the user input the btn.
void BTN_Intr_Handler(void *InstancePtr)
{
	...
	switch(state){
		...
		case CLOCK:
		{
			if(btn_value == C)	//Go to next state
			{
			...
			else if(btn_value == A)	//Switch from 24h to 12h time format and verse visa.
			{
			...
			else if(btn_value == L)	//If the alarm sounds, this will terminate it.
			{
		...
		case ALARM_SETTING:
		{
			if(btn_value == L)	//Go to next alarm-setting sub-state
			{
				printToDisplay();
			...
			else if(btn_value == C)		//Go to next state
			{
		...
		case STOPWATCH:
		{
			...
			if(btn_value == A)	//Start the stop watch
			{
				XTmrCtr_Start(&TMRInst, 0);
				XScuGic_Enable(&INTCInst, INTC_STP_WTC_INTERRUPT_ID);	//Enable the stop watch interrupt.
				while(btn_value == A){btn_value = XGpio_DiscreteRead(&BTNInst, 1);}
		...
		{
			if(btn_value == A)	//Stop the stop watch
			{
				printToDisplay();
				XScuGic_Disable(&INTCInst, INTC_STP_WTC_INTERRUPT_ID);	//Disable the interrupt handler, so the stop watch cannot increment.
				while(btn_value == A){btn_value = XGpio_DiscreteRead(&BTNInst, 1);}
				state = STOPWATCH;
			}

			else if(btn_value == L)	//Make a lap in stop watch mode
			{
		...
		case SET_CLOCK_HOUR:
			{
				...
				{
					XTime_GetTime(&tStart);
					while(btn_value == A && duration < 2.0)	//wait, and see if the user presses A for more than 2 seconds, else just increase by one
					{
						XTime_GetTime(&tEnd);
						duration = ((double)(tEnd-tStart))/COUNTS_PER_SECOND;
						btn_value = XGpio_DiscreteRead(&BTNInst, 1);
					...
					if(btn_value == A)
					{
						while(btn_value == A)	//If a is pressed for more than 2 seconds, this loop will be active, and fast forwarding is obtained
						{
							XTime_GetTime(&tEnd);
...
void TMR_Intr_Handler(void *InstancePtr, u8 TmrCtrNumber)
{
	...
	if (TmrCtrNumber==0) { //Handle interrupts generated by timer 0
		...
		if (XTmrCtr_IsExpired(pTMRInst,0)){	//Here the time is incremented by one second - every second.
				...
				if(minut10 > 5){
					//If daily time alarm is on, then a short tone will sounds every hour.
		...
		if(state == ALARM_IS_ACTIVE){	//If alarm is active, then it will not sound for more than 20 seconds.
...
//This interrupt handler is interrupting 100 times pr. second when enabled.
void STP_WCH_TMR_Intr_Handler(void *InstancePtr, u8 TmrCtrNumber){
...
void printTime(){
	if(timemode == TWENTYFOUR_HOUR_TIME_FORMATE){
		printAsciiNumber(hour10, hour0, minut10, minut0, sec10,sec0);
	}
	else
	{
...
void printAlarm(){
	printAsciiNumber(alarmHour10, alarmHour0, alarmMinut10, alarmMinut0, alarmSec10, alarmSec0);
...
void printStopWatch(){
	printAsciiNumber(swMinut10, swMinut0, swSec10, swSec0, swCenSec10, swCenSec0);
...
//This function controls the layout of all output in every state.
void printToDisplay(){
...
void printLap(){
	printAsciiNumber(lapMinute10, lapMinute0, lapSec10, lapSec0, lapCenSec10, lapCenSec0);
...
void printAsciiNumber(int hour10, int hour0, int min10, int min0, int sec10, int sec0){
	...
	for(int i = 0 ; i < 7 ; i++)
	{
		xil_printf("%s %s %s %s %s %s %s %s ", numberAscii[getStartAddress(hour10)+i]  , numberAscii[getStartAddress(hour0)+i], colonAscii[i]  , numberAscii[getStartAddress(min10)+i] , numberAscii[getStartAddress(min0)+i], colonAscii[i] , numberAscii[getStartAddress(sec10)+i] , numberAscii[getStartAddress(sec0)+i]);
		if(i != 6){xil_printf("\n\r");}
		if(i == 6){xil_printf("%s %i %s %d\n\r", getDay(day), getDate(), getMonth(month), year);}
	}
}
...
void printTwelveHourFormat(){
	...
	if(hour10 == 0)
	{
		for(int i = 0 ; i < 7 ; i++)
			{
				xil_printf("%s %s %s %s %s %s %s %s ", numberAscii[getStartAddress(0)+i]  , numberAscii[getStartAddress(hour0)+i], colonAscii[i]  , numberAscii[getStartAddress(minut10)+i] , numberAscii[getStartAddress(minut0)+i], colonAscii[i] , numberAscii[getStartAddress(sec10)+i] , numberAscii[getStartAddress(sec0)+i]);
				if(i != 6){xil_printf("\n\r");}
				if(i == 6){xil_printf("AM %s %i %s %d\n\r", getDay(day), getDate(), getMonth(month), year);}
			}
	}

	else if(hour10 == 1 && hour0 < 2)
	{

		for(int i = 0 ; i < 7 ; i++)
			{
				xil_printf("%s %s %s %s %s %s %s %s ", numberAscii[getStartAddress(1)+i]  , numberAscii[getStartAddress(hour0)+i], colonAscii[i]  , numberAscii[getStartAddress(minut10)+i] , numberAscii[getStartAddress(minut0)+i], colonAscii[i] , numberAscii[getStartAddress(sec10)+i] , numberAscii[getStartAddress(sec0)+i]);
				if(i != 6){xil_printf("\n\r");}
				if(i == 6){xil_printf("AM %s %i %s %d\n\r", getDay(day), getDate(), getMonth(month), year);}
			}
	}
	...
	else if(hour10 == 1 && hour0 >= 2)
		{
			...
			for(int i = 0 ; i < 7 ; i++)
				{
					xil_printf("%s %s %s %s %s %s %s %s ", numberAscii[getStartAddress(0)+i]  , numberAscii[getStartAddress(hour12format)+i], colonAscii[i]  , numberAscii[getStartAddress(minut10)+i] , numberAscii[getStartAddress(minut0)+i], colonAscii[i] , numberAscii[getStartAddress(sec10)+i] , numberAscii[getStartAddress(sec0)+i]);
					if(i != 6){xil_printf("\n\r");}
					if(i == 6){xil_printf("PM %s %i %s %d\n\r", getDay(day), getDate(), getMonth(month), year);}
				}
		}
...
int getStartAddress(int number){
	switch(number)
		...
		case 0:
		{
			return START_ADDRESS_0;
			break;
		}
		case 1:
			{
				return START_ADDRESS_1;
				break;
			}
		case 2:
			{
				return START_ADDRESS_2;
				break;
			}
		case 3:
			{
				return START_ADDRESS_3;
				break;
			}
		case 4:
			{
				return START_ADDRESS_4;
				break;
			}
		case 5:
			{
				return START_ADDRESS_5;
				break;
			}
		case 6:
			{
				return START_ADDRESS_6;
				break;
			}
		case 7:
			{
				return START_ADDRESS_7;
				break;
			}
		case 8:
			{
				return START_ADDRESS_8;
				break;
			}
		case 9:
			{
				return START_ADDRESS_9;
				break;
			}
		default: return 0;
	}
